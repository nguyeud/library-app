...
function showEmpty() {
    if (
        myLibrary.length == 0 ||
        JSON.parse(window.localStorage.getItem("myLibrary")) == null
    ) {
...
function addTag(e) {
    if (e.key == "Enter") {
        ...
        if (tag.length >= 1 && !tags.includes(tag)) {
            // if tag length is greater than 1 AND the tag doesn't exist
...
function displayBook(myLibrary) {
    for (let i = 0; i < myLibrary.length; i++) {
        ...
        // Create book HTML
        let bookHTML = `<div class="collection" id="${id}">
    ...
    deleteBtn.forEach((book) => {
        book.addEventListener("click", (e) => {
            // get title of book
            let titleID = e.target.id.slice(
                e.target.id.indexOf("-") + 1,
                e.target.id.length
            );
            let title = e.target.id
                .slice(e.target.id.indexOf("-") + 1, e.target.id.length)
                .replace(/SPACESPACE/g, " ");
            // load library
...
window.addEventListener("load", (event) => {
    // Load books from local storage if any
    window.localStorage.setItem("myLibrary", JSON.stringify(myLibrary));
